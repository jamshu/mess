...
// Odoo only accepts an email as a login. Let users sign up / log in with a plain
// username or mobile number by appending the instance's own domain (e.g.
// "john" or "+91 98765 43210" → john@messapp.odoo.com). Anything already
// containing "@" is treated as a real email and left alone. Used by BOTH signup
// and login so the two always resolve to the same Odoo login.
export function loginDomain() {
	try {
		return new URL(env.ODOO_URL).host; // e.g. messapp.odoo.com
	} catch {
		return 'localhost';
	}
}
...
export function normalizeLogin(raw) {
	...
	// username or mobile: keep safe local-part chars, strip the rest (spaces, +, -)
	const local = v.replace(/[^a-z0-9._-]/g, '');
	return local ? `${local}@${loginDomain()}` : v;
}
...
/**
 * Hard-delete a user and everything they own: their expenses (with all comments
 * on them), their comments on other expenses, expense groups they created,
 * settlements they are part of, push subscriptions, and group memberships.
 * Authorization is the caller's responsibility.
 *
 * NOTE: leaves them referenced as payer/participant on OTHER users' expenses —
 * removing them there would silently rewrite shared history and other people's
 * balances. A departing member should settle up first.
 */
export async function deleteUserAccount(uid) {
	// drop them from expense-group memberships
	const memberOf = await adminExecute('x_expense_group', 'search', [
		[['x_studio_member_ids', 'in', [uid]]]
	]);
	if (memberOf.length) {
		await adminExecute('x_expense_group', 'write', [memberOf, { x_studio_member_ids: [[3, uid]] }]);
	}

	// their expenses, including everyone's comments on those expenses
	const expenses = await adminExecute('x_expense', 'search', [[['create_uid', '=', uid]]]);
	if (expenses.length) {
		const cmts = await adminExecute('x_expense_comment', 'search', [
			[['x_studio_expense_id', 'in', expenses]]
		]);
		if (cmts.length) await adminExecute('x_expense_comment', 'unlink', [cmts]);
		await adminExecute('x_expense', 'unlink', [expenses]);
	}

	// their comments on other people's expenses
	const comments = await adminExecute('x_expense_comment', 'search', [[['create_uid', '=', uid]]]);
	if (comments.length) await adminExecute('x_expense_comment', 'unlink', [comments]);

	// settlements they sent or received, and groups + push subscriptions they own
	const settlements = await adminExecute('x_settlement', 'search', [
		['|', ['x_studio_from_id', '=', uid], ['x_studio_to_id', '=', uid]]
	]);
	if (settlements.length) await adminExecute('x_settlement', 'unlink', [settlements]);
	const ownGroups = await adminExecute('x_expense_group', 'search', [[['create_uid', '=', uid]]]);
	if (ownGroups.length) await adminExecute('x_expense_group', 'unlink', [ownGroups]);
