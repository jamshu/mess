...
export async function POST({ request, cookies }) {
	try {
		...
		if (!name || !email || !password) {
			return json({ ok: false, error: 'Name, email/username and password are required' }, { status: 400 });
		}
		// A plain username or mobile becomes user@<instance domain>; a real email
		// is kept as-is. Validate the resulting login is a well-formed address.
		const login = normalizeLogin(email);
		if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(login)) {
			return json({ ok: false, error: 'Enter a valid email, username, or mobile number' }, { status: 400 });
		}
		if (String(password).length < 6) {
			return json({ ok: false, error: 'Password must be at least 6 characters' }, { status: 400 });
		}

		await signupUser({ name, email: login, password, org });

		// log the new user straight in (pending users can log in; the data API
		// rejects them until an org admin approves)
		const { sessionId, info } = await authenticateUser(login, password);
